# Reusable GitHub Actions and Workflows for Onlishop extensions
...
jobs:
    cs:
        uses: onlishop/github-actions/.github/workflows/cs-fixer.yml@main
```

#### phpstan

Installs Onlishop 6, PHPStan and the extension, then runs PHPStan

```yaml
jobs:
    phpstan:
        uses: onlishop/github-actions/.github/workflows/phpstan.yml@main
        with:
          # Extension name
          extensionName: MyExtensionName
          # Run against Onlishop version
          onlishopVersion: 6.5.x
```
...
jobs:
    phpstan:
        uses: onlishop/github-actions/.github/workflows/admin-eslint.yml@main
        with:
          # Extension name
          extensionName: MyExtensionName
          # Run against Onlishop version
...
jobs:
    admin-jest:
        uses: onlishop/github-actions/.github/workflows/admin-jest.yml@main
        with:
            # Extension Name
            extensionName: MyExtensionName
            # Run against Onlishop version
...
jobs:
  phpunit:
    uses: onlishop/github-actions/.github/workflows/phpunit.yml@main
    with:
      # Extension Name
      extensionName: MyExtensionName
      # Run against Onlishop version
...
Builds the extension zip and validates the zip using onlishop-cli

```yaml
jobs:
  zip:
    uses: onlishop/github-actions/.github/workflows/build-zip.yml@main
    with:
      # Extension Name
      extensionName: MyExtensionName
```
...
Upload the extension with the given Onlishop account credentials into the account. It is recommended to use the `workflow_dispatch` event, so you have to manually trigger this from the Actions Tab.
...
jobs:
  build:
    uses: onlishop/github-actions/.github/workflows/store-release.yml@main
    with:
      extensionName: ${{ github.event.repository.name }}
    secrets:
      accountUser: ${{ secrets.ONLISHOP_ACCOUNT_USER }}
      accountPassword: ${{ secrets.ONLISHOP_ACCOUNT_PASSWORD }}
...
jobs:
    phpstan:
        ...
        with:
          ...
          dependencies: |-
            [
              {"name": "SwagPlatformDemoData", "repo": "git@github.com:onlishop/SwagPlatformDemoData.git"}
            ]
```
...
jobs:
  downstream:
    ...
    steps:
      - uses: onlishop/github-actions/downstream@main
        with:
          repo: onlishop/actions-test
          workflow: test
          ref: trunk
```
  ...
  id:
    ...
    steps:
      - if: ${{ inputs.upstream_data }}
        uses: onlishop/github-actions/upstream-connect@main
        with:
...
subject: repo:onlishop/onlishop:ref:refs/heads/main
# you can also use subject_pattern, if you want to use regex

claim_pattern:
  # restrict to a specificy upstream workflow
  job_workflow_ref: onlishop/onlishop/.github/workflows/downstream.yml@refs/heads/.*
...
This policy only allows the `main` ref with the `downstream.yml` workflow of the `onlishop/onlishop` repository to get a token with the `actions:write` permissions.
